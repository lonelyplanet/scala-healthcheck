...
case class DatabaseServiceDependency(override val id: String)
    extends ServiceDependency(ServiceDependency.DatabaseType, id)
...
object DatabaseDependency {
  def apply(databaseServiceDependency: DatabaseServiceDependency,
            databaseHealth: DatabaseHealth): DatabaseDependency = {
    val status =
      if (databaseHealth.isConnectable) DependencyStatusGreen
      else DependencyStatusRed
    DatabaseDependency(
  ...
  def asResourceObject(o: DatabaseDependency): ResourceObject = ResourceObject(
      `type` = o.`type`,
      id = Some(o.id),
      attributes = Some(
          List(
...
case class DatabaseHealth(location: String,
...
object DatabaseHealth {
  ...
  def apply(databaseHost: String,
            databasePort: Int,
            isConnectable: Boolean): DatabaseHealth = {
    val message =
      if (isConnectable) DatabaseConnectableMessage
      else DatabaseNotConnectableMessage
